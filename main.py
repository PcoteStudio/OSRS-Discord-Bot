...
def getAllBossesTopX(x: int):
    ...
    for i, metric in enumerate(boss_metrics):
        ...
        bosses_topX.append(
            {'displayName': boss_display_names[i], 'topX': topX})
    return bosses_topX


def formatBossesTop1(all_bosses_top1: list):
...
class BotClient(discord.Client):
    ...
    async def update_hs_channel(self):
        ...
        try:
            ...
            topX = getAllBossesTopX(secrets['TOP_X'])
            content = formatBossesTop1(
                topX) if secrets['TOP_X'] == 1 else formatBossesTopX(topX)
            parts = content.split(" ```\n")
            content = []
            for part in parts:
