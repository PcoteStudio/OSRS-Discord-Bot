...
from internal.sal.tilenode import TileNode, TileType, Destination
...
class Team:
    ...
    def set_start_tile(self, tiles, tile, min_time):
        ...
        self.choose_destination(dest, min_time, tiles)
    ...
    def erase_rolled_back_history(self):
        while self.history_index < len(self.history) - 1:
            self.history.pop()

    def choose_destination(self, destination, min_time, tiles):
        ...
        if (destination.move):
            ...
            new_destinations = self.choose_destination(move_destinations[0], min_time, tiles)
            return [destination] + new_destinations
        return [destination]

    def is_tile_done(self, destination):
        for i in range(self.history_index):
            h = self.history[i]
            tile_index = h["tile_index"]
            if (destination.index == tile_index):
                return True
        return False
