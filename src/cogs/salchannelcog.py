...
from internal.sal import snakesandladders, salchecks
from database.sal import snakesandladdersdb, teamdb
...
class ChannelCog(commands.Cog):
    ...
    async def logs(self, interaction: nextcord.Interaction, channel: nextcord.TextChannel):
        ...
        logging.info(
            f"{utils.format_guild_log(interaction.guild)} SaL logs channel has been set to {channel.name} by {interaction.user.name})")
    ...
    async def board(self, interaction: nextcord.Interaction, channel: nextcord.TextChannel):
        ...
        logging.info(
            f"{utils.format_guild_log(interaction.guild)} SaL live board channel has been set to {channel.name} by {interaction.user.name})")
    ...
    @channel.subcommand(description="Unset the current SaL channels.")
    @application_checks.has_role(constants.ROLE_BOT_ADMIN)
    @salchecks.game_exists()
    async def unset(self, interaction: nextcord.Interaction):
        ...
        logging.info(
            f"{utils.format_guild_log(interaction.guild)} SaL channels have been unset by {interaction.user.name})")
