from database import tilenodedb
...
async def get_by_guild_id(guild_id: int):
    doc = await instance.game_of_life_doc.find_one({'guild_id': guild_id, 'is_archived': False})
    if doc is None:
        return None
...
async def insert(game: GameOfLife):
    ...
    return await instance.game_of_life_doc.insert_one(doc)


async def archive(game: GameOfLife):
    game.is_archived = True
    result = await instance.game_of_life_doc.update_one({'_id': game._id}, {'$set': {'is_archived': game.is_archived}})
    return result
